...
if(rs.next()){
	...
	response.sendRedirect("sis_update_produtos.jsp?msg=5&produtoID="+produto.produtoID);
}else{
	...
	if(rs01.next()){
		...
		response.sendRedirect("sis_update_produtos.jsp?msg=8&produtoID="+produto.produtoID);
	}else{
		//Senão: Executa a Função que irá Alterar os dados na Base de Dados
		st02.execute(produto.alteraProduto());
		response.sendRedirect("sis_view_produtos_mix_publicidade.jsp?msg=2");
	}
}
%>
