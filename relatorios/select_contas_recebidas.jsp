...
function abreRelatorio(diaA, mesA, anoA, diaB, mesB, anoB, forma, clienteID){
	window.open('rep_contas_recebidas.jsp?diaA='+diaA+'&mesA='+mesA+'&anoA='+anoA+'&diaB='+diaB+'&mesB='+mesB+'&anoB='+anoB+'&forma='+forma+'&clienteID='+clienteID, 'Retiradas', 'width = 800; height = 600; scrollbars=yes')
}
...
<form name="form1" method="post">
 <table align="center" width="819">
  <tr bgcolor="#EEEEEE">
   <td height="35" colspan="9" align="center"><strong>PESQUISE TODAS AS CONTAS A RECEBIDAS DENTRO DE UM PER&Iacute;ODO</strong></td>
  </tr>
  <tr>
    ...
    <select name="clienteID" style="width:180px">
     <option value="0" selected="selected">Todos os Clientes</option>
     <%while(rs02.next()){ %>
...
%>
 <script type="text/javascript">
  abreRelatorio('<%=request.getParameter("diaA")%>', '<%=request.getParameter("mesA")%>', '<%=request.getParameter("anoA")%>', '<%=request.getParameter("diaB")%>', '<%=request.getParameter("mesB")%>', '<%=request.getParameter("anoB")%>', '<%=request.getParameter("formaID")%>', '<%=request.getParameter("clienteID")%>');
 </script>
<%}%>
