...
if(rs.next()){
	...
	response.sendRedirect("sis_insert_produtos_mix_publicidade.jsp?msg=4");
}else{
	...
	if(rs01.next()){
		...
		response.sendRedirect("sis_insert_produtos_mix_publicidade.jsp?msg=7");
	}else{
		//Senão: Executa a Função que irá salvar os dados na Base de Dados
		st02.execute(produto.salvaProduto());
		response.sendRedirect("sis_view_produtos_mix_publicidade.jsp?msg=1");
	}
}
%>
